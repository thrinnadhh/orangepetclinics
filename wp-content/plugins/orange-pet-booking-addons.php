...
<body class="bg-gray-50 m-0 p-0">
  ...
  <script>
    ...
    function parseTimeStr(timeStr) {
      // Converts "10:00 AM" to hours (24h format) for calculation
    ...
    async function fetchAppointment() {
      ...
      try {
        ...
        const booking = result.data; // Expected: { id: "123", date: "2026-12-01", time: "10:00 AM" }
        globalRescheduleBookingId = booking.id;
        globalRescheduleType = booking.type || 'grooming';

        // ─── 1.5 HOUR Cutoff Logic ───
        const now = new Date();
        const appointmentDateStr = booking.date; // YYYY-MM-DD
        const appointmentTimeStr = booking.time; // "10:00 AM"

        // Parse the exact datetime of the appointment
        ...
        if (diffHours < 1.5) {
          // Rule applied: Cannot change strictly less than 1.5 hours before slot
